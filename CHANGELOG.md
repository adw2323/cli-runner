...
### Added
- Agent-agnostic adapter architecture in `src/cli_ai_runner/adapters/`.
- Built-in adapters for `codex`, `gemini`, and `claude`.
- `cli-ai-runner status` command for agent detection.
- `cli-ai-runner setup` command for automated agent CLI installation.
- GitHub Actions CI for Windows with Python 3.11/3.12.
- PyPI release workflow using trusted publishers (OIDC).

### Changed
- Renamed project from `cli-orchestrator-ui` to `cli-ai-runner`.
- Simplified core execution loop in `runner.py`.
- Replaced hardcoded `codex exec` with dynamic adapter invocations.
- Moved utility functions to `src/cli_ai_runner/utils.py`.
