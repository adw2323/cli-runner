# cli-ai-runner User Guide

`cli-ai-runner` is a powerful, deterministic task execution engine designed to manage the lifecycle of agentic CLI workflows. It transforms unpredictable AI outputs into stable, repeatable project progression.

## Core Concepts

### The Deterministic Loop
Unlike raw CLI tools, `cli-ai-runner` expects agents to communicate their status. By looking for the `RUN_STATUS` token, the runner decides whether to finish, continue, or rework a task.
...
When `--strict-completion` is enabled (default), `cli-ai-runner` won't trust a `DONE` signal immediately. It will first force the agent into a "Verification Phase" where it must check:
...
```powershell
cli-ai-runner run "Implement the checkout logic" --agent claude --max-loops 10
```
...
cli-ai-runner setup --agent claude --dry-run
```
...
If `cli-ai-runner status` shows an agent as missing but you can run it manually:
1. Ensure the binary is in your system `PATH`.
2. On Windows, `cli-ai-runner` prefers `.cmd` or `.bat` wrappers. Check if you have those available.
3. Try restarting your terminal after running `cli-ai-runner setup`.
