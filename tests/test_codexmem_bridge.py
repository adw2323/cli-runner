...
async def test_load_recent_lines_filters_repo_and_formats(monkeypatch: pytest.MonkeyPatch) -> None:
    payload = {
        ...
        "items": [
            ...
            },
            {
                ...
                "repoId": "cli-agent-orchestrator-ui",
                "cwd": "C:\\Users\\andrew.walsh\\cli-agent-orchestrator-ui",
    ...
    monkeypatch.setattr("cli_orchestrator_ui.broker.codexmem.subprocess.run", _run)
    bridge = CodexMemBridge(
        cwd=Path("C:\\Users\\andrew.walsh\\cli-agent-orchestrator-ui"),
        repo_id="cli-agent-orchestrator-ui",
    )
...
async def test_add_run_invokes_cli(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("cli_orchestrator_ui.broker.codexmem.subprocess.run", spy)
    bridge = CodexMemBridge(
        cwd=Path("C:\\Users\\andrew.walsh\\cli-agent-orchestrator-ui"),
        repo_id="cli-agent-orchestrator-ui",
        branch="master",
    ...
    assert "cli-agent-orchestrator-ui" in args
...
def test_queue_add_run_no_event_loop_noop(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("cli_orchestrator_ui.broker.codexmem.asyncio.get_running_loop", Mock(side_effect=RuntimeError()))
...
def test_queue_add_run_schedules_task(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("cli_orchestrator_ui.broker.codexmem.asyncio.get_running_loop", Mock(return_value=fake_loop))
    bridge.queue_add_run(request="x", summary="y", status="running")
...
async def test_run_cli_json_handles_error_and_invalid_output(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("cli_orchestrator_ui.broker.codexmem.subprocess.run", raise_run)
    assert await bridge._run_cli_json(["journal-list"]) is None

    monkeypatch.setattr("cli_orchestrator_ui.broker.codexmem.subprocess.run", Mock(return_value=_Result(1, "{}")))
    assert await bridge._run_cli_json(["journal-list"]) is None

    monkeypatch.setattr("cli_orchestrator_ui.broker.codexmem.subprocess.run", Mock(return_value=_Result(0, "not-json")))
    assert await bridge._run_cli_json(["journal-list"]) is None
...
def test_matches_repo_and_format_helpers() -> None:
    ...
    assert bridge._matches_repo({"cwd": "C:\\Users\\andrew.walsh\\cli-agent-orchestrator-ui\\subdir"}) is True
