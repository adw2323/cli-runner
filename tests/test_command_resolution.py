...
from cli_orchestrator_ui.broker import engine as engine_module
from cli_orchestrator_ui.broker.engine import _resolve_command, _should_use_pty_for_command


def test_resolve_command_uses_shutil_which(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("cli_orchestrator_ui.broker.engine.shutil.which", lambda program: "C:\\x\\tool.cmd")
...
def test_resolve_command_windows_extension_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("cli_orchestrator_ui.broker.engine.shutil.which", fake_which)
