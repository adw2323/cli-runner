...
def test_run_task_loop_done_status_single_run() -> None:
    with patch(
        "cli_runner.runner._run_codex_once",
...
def test_run_task_loop_rework_returns_nonzero() -> None:
    with patch("cli_runner.runner._run_codex_once", return_value=(0, "RUN_STATUS:REWORK\n")):
        result = run_task_loop("do work", codex_cmd="codex exec", max_loops=4, strict_completion=False)
...
def test_run_task_loop_hits_loop_limit() -> None:
    with patch("cli_runner.runner._run_codex_once", return_value=(0, "RUN_STATUS:CONTINUE\n")):
...
def test_run_task_loop_resolves_command_before_execution() -> None:
    with (
        patch("cli_runner.runner._resolve_command", return_value=["C:\\tools\\codex.cmd", "exec"]) as resolve_mock,
        patch("cli_runner.runner._run_codex_once", return_value=(0, "RUN_STATUS:DONE\n")) as run_once_mock,
    ):
        result = run_task_loop("do work", codex_cmd="codex exec", max_loops=2, strict_completion=False)
    assert result.status == "done"
